...
import soya.framework.nezha.support.TaskDefinition;
import soya.framework.pipeline.Pipeline;
import soya.framework.pipeline.PipelineInitializeException;
...
public class NezhaPipeline implements Pipeline, Workflow {
    ...
    private NezhaPipeline(File base) {
        this.base = base;
    ...
    public static NezhaPipeline fromDir(File base) throws IOException {
        if (new File(base, "pipeline.yaml").exists()) {
            File ppl = new File(base, "pipeline.yaml");
            return fromYaml(base, new FileInputStream(ppl));

        } else if (new File(base, "pipeline.json").exists()) {
            return fromJson(base, new FileInputStream(new File(base, "pipeline.json")));
    ...
    public static NezhaPipeline fromJson(File base, InputStream inputStream) {
        Builder builder = NezhaPipeline.builder(base);

        return builder.create();
    }

    public static NezhaPipeline fromYaml(File base, InputStream inputStream) {
        ...
        configuration.entrySet().forEach(e -> {
            ...
            switch (key) {
                ...
                case "main-flow":
                    ...
                    break;
                case "scheduler":
                    builder.scheduler(parse(e.getValue(), NezhaPipeline.Scheduler.class, base));
                    break;
                default:
                    ;
            }
    ...
    static class Builder {
        ...
        public NezhaPipeline create() {
            NezhaPipeline pipeline = new NezhaPipeline(base);
